...
vector<vector<long double>> ClampedCubicSpline(vector<pair<long double, long double>> point, long double FPO, long double FPN)
{
	...
	vector<long double> h(n - 1), alpha(n), l(n), muy(n - 1), z(n);
	...
	//Step 2
	alpha[0] = 3.0 * (point[1].second - point[0].second) / h[0] - 3.0 * FPO;
	alpha[n - 1] = 3.0 * FPN - 3.0 * (point[n - 1].second - point[n - 2].second) / h[n - 2];

	//Step 3
	for (int i = 1; i < n - 1; i++) {
		alpha[i] = 3.0 * (point[i + 1].second - point[i].second) / h[i] - 3.0 * (point[i].second - point[i - 1].second) / h[i - 1];
	}

	//Step 4
	l[0] = 2.0 * h[0]; muy[0] = 0.5; z[0] = 1.0 * alpha[0] / l[0];

	//Step 5
	for (int i = 1; i < n - 1; i++) {
		l[i] = 2.0 * (point[i + 1].first - point[i - 1].first) - h[i - 1] * muy[i - 1];
		muy[i] = h[i] / l[i];
		z[i] = (alpha[i] - h[i - 1] * z[i - 1]) / l[i];
	}

	//Step 6
	l[n - 1] = h[n - 2] * (2.0 - muy[n - 2]); 
	...
	for (int i = n - 2; i >= 0; i--) {
		...
		solution[i][2] = z[i] - muy[i] * c;
		solution[i][1] = (point[i + 1].second - point[i].second) / h[i] - h[i] * (c + 2 * solution[i][2]) / 3.0;
		solution[i][3] = (c - solution[i][2]) / 3.0 / h[i];
	}
